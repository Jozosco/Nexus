...
수동 갱신본(9/13 업로드)과 봇 append 행을 26파일 전부 대조한 결과 동일 일자 종가 5건이 0.000% 일치하고 구조 결함이
없어, **9/11 이후 일별 갱신은 `te_xlsx_update.yml` 스냅샷 cron(평일 21:30 UTC, `--mode snapshot`) 단일 경로**로
확정함(대조 기록: `docs/research_desk/2026-09/te_manual_vs_api_check_2026_09_13.md`).

- **수동 재업로드는 봇이 덧붙인 최신 행을 되돌릴 수 있음**(9/13 업로드에서 Urea·CRB·EU Carbon·GSCI·CFI 5파일이 봇
  9/10~9/11 행을 되돌림). 갱신기는 파일 마지막 일자 이후만 append하므로 다음 런이 자동으로 다시 붙임 — 손실은 없으나
  **재업로드는 공백 복구·계열 교정 목적에만** 사용함.
- **BDI 정정 확정(2026-09-13, 승인자 확인)**: 구 수동본의 **2023-01-03 이후** 계열은 오류였고 9/13 재업로드본(커밋 `6b040fc`)이
  정본임(CONFIRMED — 차이 860행 · 배율 중앙값 2.7배 · 2022-12-23 이전은 동일). 봇 점프 게이트가 차단했던 API 값과 같은 척도이므로
  이후 스냅샷은 정상 append됨. 2026년 결측 3일(4/3·4/6·5/4)은 영국 공휴일(발틱거래소 무발표)로 **정상 — 백필하지 않음**.
  실측·z 델타·영향 산출물: `docs/research_desk/2026-09/bdi_series_correction_errata_2026_09_13.md`.
- **심볼 미확정(TTF · Drewry WCI · DAP)**: 고정 심볼이 빈 스냅샷을 반환해 다중 검색어 폴백을 도입함. 다음 런의
  보고서 하단(시도 심볼·검색어 목록)과 `[정보]` 로그로 유효 심볼을 확정한 뒤 승인 후 고정함.
...
- 2026-09-13: 수동 갱신본 ↔ API 부착분 대조 검증 — 일치, 스냅샷 cron 단일 경로 확정 · 다중 검색어 심볼 폴백(TTF/WCI/DAP)
- 2026-09-13: BDI 수동본 정정 확정(승인자) — 구 계열 2023-01-03 이후 오류, 재업로드본(`6b040fc`) 정본. 정정본 기준 9/9 표준화 지수 +2.12(G1)·+2.74(도착가) → 운임 레짐 평시→급등. 공휴일 결측 3일 백필 안 함. 정오표 `bdi_series_correction_errata_2026_09_13.md`
