...
| DQ-3 | **GPT 교차검증 모델 ID 확정** | cross_verify.yml 기본값 `gpt-5.6-sol` — OpenAI API 정확한 식별자와 다르면 검증이 경고 후 skip(비차단) | Actions → Variables → `GPT_XVERIFY_MODEL`에 정확한 ID 입력 | 무료 | **즉시** — 입력 전까지 교차검증이 실호출되지 않을 수 있음 |
