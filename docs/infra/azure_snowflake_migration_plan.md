...
> ## ⛔ 2026-09-02 폐기 — 클라우드 대통합 상세 수령(Azure 전면 폐지, Power BI 제외)
> 본 문서의 **B안(Azure Blob 직행)은 폐기**됐다. 데이터 착지는 **AWS S3 + Snowflake EDP 전용 DB**로
> 직행하며, 이관 판정·지원 범위·정보 요청은 `docs/research_desk/2026-09/cloud_integration_impact_2026_09_02.md`가
> 정본이다. 아래 본문은 이력 보존용(스냅샷 불변·`snapshot_date` 경로 규약 2가지는 S3에도 그대로 승계).
